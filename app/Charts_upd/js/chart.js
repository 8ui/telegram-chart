function Chart(el, data, species) {
	...
	this.init = function() {
		...
		OverlayCTX = OverCanvas.getContext('2d')

		AxisCanvas = dom('canvas', ChartWr, {
			className: 'axis',
			width: W,
			height: xAxisH + H + headerH
		})
	...
	function drawTooltip() {
				...
				if (series[i].active) {
					if (indexes[ind] === undefined) items[ind].push([])
						...
						items[ind].push([
							...
							parseInt(series[i].valumes[indexes[ind]] * 100) + '%',
						])
					}
		...
		if (type !== 'area' && items[0].length > 1 && !yscaled) {
	...
	function render(t) {
		...
		for (var i = 0; i < YdiffAxis.length; i++) {
			...
			if (updateAnimation(animMinY[i])) drawMain = drawPreview = true;
